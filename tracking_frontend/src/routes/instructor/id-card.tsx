...
function InstructorIdCard() {
  ...
  const [accountCreatedAt, setAccountCreatedAt] = useState<string | null>(user?.createdAt ?? null);

  useEffect(() => {
    if (!accountCreatedAt) {
      api.get<{ user: { createdAt?: string } }>("/auth/me").then((d) => {
        if (d.user?.createdAt) setAccountCreatedAt(d.user.createdAt);
      }).catch(() => {/* silent */});
    }
  }, []);
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      pdf.save(`${user?.name ?? "instructor"}-id-card.pdf`);
  ...
  // Issue = account creation, Expiry = 1 year from creation
  const issueDate = user?.createdAt
    ? new Date(user.createdAt).toLocaleDateString([], { month: "short", day: "numeric", year: "numeric" })
    : "—";
  const expiryDate = user?.createdAt
    ? (() => { const d = new Date(user.createdAt!); d.setFullYear(d.getFullYear() + 1); return d.toLocaleDateString([], { month: "short", day: "numeric", year: "numeric" }); })()
    : "—";
