...
export const Route = createFileRoute("/parent/edit-profile")({
  head: () => ({ meta: [{ title: "Edit Profile | CodeCampus Parent Portal" }] }),
...
function ParentEditProfile() {
  ...
  const isGoogleUser = !!(user as { googleId?: string } | null)?.googleId;
  ...
  const handleImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const token = (() => {
        try {
          const r = localStorage.getItem("excellence_auth");
          return r ? (JSON.parse(r) as { token: string }).token : null;
        } catch { return null; }
      })();
